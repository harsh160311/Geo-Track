"""
GeoTrace Backend Server v2 — Session Mode
...
# ── SESSION STORE ──────────────────────────────────────
# { token: { "captures": [...], "created_at": float, "last_seen": float } }
sessions = {}
SESSION_TTL = 5 * 60  # 5 minutes

def cleanup_sessions():
    """Background thread — deletes sessions inactive for 5 min"""
    while True:
        time.sleep(30)
        now = time.time()
        expired = [t for t, s in list(sessions.items()) if now - s["last_seen"] > SESSION_TTL]
        for t in expired:
            del sessions[t]
            print(f"[Session] Expired & deleted: {t[:8]}...")
